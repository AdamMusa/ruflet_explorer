...
# === gallery/sections_controls/calculator.rb ===
module Gallery
  module SectionsControls
    ...
    def build_calculator(page, status)
      ...
      calculator_width = preview_content_width(page, max: 420, margin: compact ? 32 : 56, min: 280)
      key_width = [[(calculator_width - 42) / 4.0, 56].max, 78].min
      preview_height = preview_content_height(page, max: 500, chrome: 170, min: 360)
      key_height = compact && preview_height < 520 ? 40 : (compact ? 48 : 65)
      container(
        width: calculator_width,
        height: compact ? preview_height : nil,
        padding: 12,
        border_radius: 12,
    ...
    def calculator_display(page)
      @calculator_display = text(
        ...
        text_align: "right",
        style: { size: page.width.to_f < 700 ? 56 : 84, color: color_text(page) }
      )
    end
