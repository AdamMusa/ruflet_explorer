...
def calculator_display(page, compact)
  @calculator_display = text(
    value: calculator_state[:display],
    text_align: "right",
    style: { size: compact ? 56 : 84, color: "#111827" }
  )
end
...
def calculator_keypad_row(page, display, status, key_width, key_height, *labels)
  row(
    alignment: "center",
    spacing: 6,
...
Ruflet.run do |page|
  ...
  page.bgcolor = "#ffffff"
  compact = page.width.to_f.positive? && page.width.to_f < 700
  calculator_width = if page.width.to_f.positive?
                       [[page.width.to_f - (compact ? 32 : 56), 280].max, 420].min
                     else
                       420
                     end
  preview_height = page.height.to_f
  compact_panel_height = [[preview_height - 24, 360].max, 500].min
  compact_key_height = preview_height.positive? && preview_height < 520 ? 40 : 48
  key_width = [[(calculator_width - 42) / 4.0, 56].max, 78].min
  key_height = compact ? compact_key_height : 65
  status = text(value: "", style: { size: 12, color: "#6b7280" })
  display = calculator_display(page, compact)
  page.add(
    container(
      expand: true,
      alignment: "center",
      ...
      content: container(
        ...
        height: compact ? compact_panel_height : nil,
        padding: 12,
        border_radius: 12,
        bgcolor: "#f3f4f6",
        content: column(
          ...
          children: [
            ...
            calculator_keypad_row(page, display, status, key_width, key_height, "BS", "AC", "%", "/"),
            calculator_keypad_row(page, display, status, key_width, key_height, "7", "8", "9", "x"),
            calculator_keypad_row(page, display, status, key_width, key_height, "4", "5", "6", "-"),
            calculator_keypad_row(page, display, status, key_width, key_height, "1", "2", "3", "+"),
            calculator_keypad_row(page, display, status, key_width, key_height, "+/-", "0", ".", "=")
          ]
        )
      )
    )
